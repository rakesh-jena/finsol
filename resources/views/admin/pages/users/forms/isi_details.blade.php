@if ($usersIso)
    <div class="row mb-4">
        <h5>ISI Form Details</h5>
        <div class="col-12">
            <div id="tableExample" data-list='{"valueNames":["type","email","company"],"page":15,"pagination":true}'>
                <div class="table-responsive scrollbar">
                    ...
                    <table class="table table-bordered table-striped fs--1 mb-0">
                        ...
                        <tbody class="list">
                            @if ($usersIsi)
                                @foreach ($usersIsi as $detail)
                                    <tr class="align-middle" data-toggle="collapse" class="accordion-toggle">
                                        <td class="text-nowrap">{{ $detail->email_id ? $detail->name_of_company : '--' }}</td>
                                        ...
                                        <td><a
                                                href="{{ url('admin/user/forms/details/isi/' . $detail->id) }}">Details</a>
                                        </td>
                                        <td>
                                                    ...
                                                    <div><span
                                                        ...
                                                        @if ($detail->additional_img != '')
                                                            <form
                                                                ...
                                                                <input type="hidden" name="form_type" value="ISI">
                                        ...
                                        @if ($detail->status == 1 || $detail->status == 3)
                                            <td colspan=6 style="display:flex;">
                                                <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                    type="button" onclick="openIsiNoteModal({{ $detail->id }})"
                                                    href="{{ url('forms/statusview/' . $detail->id) }}"
                                                    data-target="#myIsiNoteModal">
                                                ...
                                                <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                    ...
                                                    onclick="openIsiApproveModal({{ $detail->id }})"
                                                    data-target="#myIsiApprovedModal">
...
<script>
    ...
    function openIsiNoteModal(itemId) {
        $.ajax({
            url: adminUrl + '/user/forms/statusview' + '?for=note&formtype=isi&id=' + itemId,
            type: 'GET',
    ...
    function openIsiApproveModal(itemId) {
        $.ajax({
            url: adminUrl + '/user/forms/statusview' + '?for=approve&formtype=isi&id=' + itemId,
            type: 'GET',
            success: function(data) {
