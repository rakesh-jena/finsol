@if (count($userISIDetail) > 0)
    ...
    <table class="table table-condensed table-striped mt-2 mb-4 w-100">
        ...
        <tbody>
            @if ($userISIDetail)
                @foreach ($userISIDetail as $key => $detail)
                            ...
                            @if ($detail->payment_status == 'Credit')
                                ...
                                @else
                                    <form action="{{ route('dashboard.register') }}" method="POST">
                                        ...
                                        <input type="hidden" name="payment_amount" value="{{ $isi_instamojo_amount }}">
                                        <input type="hidden" name="route" value="form_dashboard">
                                        <input type="hidden" name="payment_purpose" value="ISI">
