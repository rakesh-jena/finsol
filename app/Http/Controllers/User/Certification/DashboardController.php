...
class DashboardController extends Controller
{
    ...
    public function queryRaised(Request $request)
    {
        ...
        if ($id) {
            $useName = $userId;
            $folderName = 'public/uploads/users/' . $useName . '/Certification/' . $formType . '/AdditionalImg';
            $name = 'additional_img';
    ...
    public function raisedFile(Request $request)
    {
        ...
        if (count($commaValues) > 1) {
            foreach ($commaValues as $file) {
                $filePath = 'public/uploads/users/' . $useName . '/Certification/' . $formType . '/' . 'RaisedImg/' . $file;
                if (File::exists($filePath)) {
                    $fileContents = file_get_contents($filePath);
                    $zip->addFromString(basename($file), $fileContents);
                } else {
    ...
    public function storePaymentData($data)
    {
        ...
        if ($form_type == 'Ca') {
            UserCaDetail::where('payment_unique_id', '=', $data->payment_request_id)->update(array('payment_status' => $data->payment_status));
        } else if ($form_type == 'Networth') {
            UserNetworthDetail::where('payment_unique_id', '=', $data->payment_request_id)->update(array('payment_status' => $data->payment_status));
        } else if ($form_type == 'Turnover') {
            UserTurnoverDetail::where('payment_unique_id', '=', $data->payment_request_id)->update(array('payment_status' => $data->payment_status));
        }
        ...
        return redirect('certification/dashboard')->with('payment_success', $msg);
    }
}
